...
async function getCachedResponse() {
  ...
  if (!pendingRequest) {
    pendingRequest = buildResponse()
      .then((result) => {
        cachedResponse = result;
        cachedResponseExpiresAt = Date.now() + CURRENTLY_PLAYING_CACHE_TTL_MS;
        return result;
      })
      .finally(() => {
        pendingRequest = null;
...
const currentlyPlayingRouter = Router();

currentlyPlayingRouter.get('/', async (_request, response, next) => {
  try {
...
export default currentlyPlayingRouter;
